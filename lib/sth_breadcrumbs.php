<?php
/**
 * Breadcrumb function. Hacked from https://www.thewebtaylor.com/articles/wordpress-creating-breadcrumbs-without-a-plugin with Bootstrappy additions
 * 
 * 
 * Place into your template files with this wrapped in a php tag set sth_breadcrumbs();
 * @package sth
 */
...
function sth_breadcrumbs() {
    ...
    if ( !is_front_page() ) {
        ...
        if ( is_archive() && !is_tax() && !is_category() ) {
            ...
            echo '<li class="item-current"><span class="bread-current bread-tag">' . $terms[0]->name . '</span></li>';
